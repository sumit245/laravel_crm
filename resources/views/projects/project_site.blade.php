        ...
        <div class="d-flex">
            ...
            <form action="{{ route('sites.import', $project->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="input-group">
                    <input type="file" name="file" style="height: 40px !important;"
                        class="form-control form-control-sm" required>
                    <button type="submit" class="btn btn-sm btn-primary"
                        style="height: 40px !important; align-items:center;justify-content:center;"
                        data-toggle="tooltip" title="Import Sites">
                        <i class="mdi mdi-upload"></i><span>Import</span>
                    </button>
                </div>
            </form>
            <a href="{{ route('sites.create', ['project_id' => $project->id]) }}" class="btn btn-primary mx-1 mb-4"
        ...
        @foreach ($sites as $site)
            <tr>
                ...
                @if ($project->project_type == 0)
                    ...
                    <td>{{ $site->engineerRelation->firstName ?? '' }} {{ $site->engineerRelation->lastName ?? ' ' }}
                    </td>
                @else
                    <td>{{ $site->task_id }}</td>
                    <td>{{ $site->state }}</td>
                ...
                <td>
                    ...
                    <form action="{{ route('sites.destroy', $site->id) }}?project_id={{ $project->id }}"
