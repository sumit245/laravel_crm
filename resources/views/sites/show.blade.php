...
@section("content")
  <div class="min-vh-100 bg-gray-50 p-4">
    <!-- Header -->
    <div class="border-dark mb-4 border-2 bg-white p-4">
      ...
      <div class="mt-2">
        <p class="h5 font-weight-medium mb-0">
          @if (isset($projectId) && $projectId == 11)
            {{ $streetlight->panchayat ?? "Mohammadpur" }}, {{ $streetlight->district ?? "Lakhisarai" }} -
            {{ $streetlight->state ?? "Bihar" }}
          @else
            {{ $site->location ?? "Mohammadpur" }}, {{ $site->districtRelation->name ?? "Lakhisarai" }} -
            {{ $site->stateRelation->name ?? "Bihar" }}
          @endif
        </p>
      </div>
    </div>
    ...
    <div class="d-flex" style="gap: 1.5rem;">
      ...
      <div style="width: 4xl;">
        @if (isset($projectId) && $projectId == 11)
          ...
          @foreach ($wards as $ward)
            <div class="card border-dark ward-button mb-3 border-2" role="button" data-ward="{{ $ward }}"
      ...
      <!-- Main Content -->
      <div class="d-flex border-dark border-2 bg-white">
        <div class="p-4">
          ...
          <div class="d-flex justify-content-between align-items-start mb-4 flex-wrap">
            <div>
              @if (isset($projectId) && $projectId == 11)
                ...
                <div class="mt-3">
                  <a href="{{ route("surveyed.poles", ["ward" => "selected_ward"]) }}"
                    class="text-primary text-decoration-none" id="surveyedPolesLink">
                    <strong>Poles Surveyed: {{ $surveyedPolesCount ?? 25 }}</strong>
                  </a><br />
                  <a href="{{ route("installed.poles", ["ward" => "selected_ward"]) }}"
                    class="text-success text-decoration-none" id="installedPolesLink">
              ...
              @else
                ...
                <div class="mt-3">
                  <a href="{{ route("surveyed.poles", ["site_id" => $site->id]) }}"
                    class="text-primary text-decoration-none">
                    <strong>Poles Surveyed: {{ $surveyedPolesCount ?? 0 }}</strong>
                  </a><br />
                  <a href="{{ route("installed.poles", ["site_id" => $site->id]) }}"
          ...
          <div class="tab-content mt-1" id="myTabContent">
            <ul class="nav nav-tabs fixed-navbar-project" id="myTab" role="tablist">
              <li class="nav-item" role="presentation">
                <button class="nav-link active" id="surveyed-tab" data-bs-toggle="tab" data-bs-target="#surveyed_poles"
                  type="button" role="tab" aria-controls="surveyed_poles" aria-selected="true">
                  Surveyed Poles
                </button>
              </li>
              <li class="nav-item" role="presentation">
                <button class="nav-link" id="installed-tab" data-bs-toggle="tab" data-bs-target="#installed_poles"
                  type="button" role="tab" aria-controls="installed_poles" aria-selected="false">
                  Installed Lights
                </button>
              </li>
            </ul>
            ...
            <div class="tab-pane fade show active" id="surveyed_poles" role="tabpanel" aria-labelledby="surveyed-tab">
              <x-data-table id="mainDataTable" class="table-striped table-bordered table-sm mt-3 table">
                <x-slot:thead>
                  <tr>
            ...
            <div class="tab-pane fade" id="installed_poles" role="tabpanel" aria-labelledby="installed-tab">
              <x-data-table id="installedDataTable" class="table-striped table-bordered table-sm mt-3 table">
                <x-slot:thead>
                  <tr>
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        if (firstWard) {
          ...
          loadWardData({
            currentTarget: firstWard,
            preventDefault: () => {}
          }, firstWard.dataset.ward);
        }
      });
    </script>
  @endpush
