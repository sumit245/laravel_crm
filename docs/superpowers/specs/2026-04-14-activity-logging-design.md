...
The goal is to implement comprehensive activity logging across the entire CRM system (both Web and API controllers). Based on the agreed approach, we will strictly log **write operations** (Create, Update, Delete, Import, Dispatch, Push) and bypass simple read/search queries to ensure our Database doesn't bloat while maintaining a rigorous audit trail of all state mutations.
