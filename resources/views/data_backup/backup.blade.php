            ...
            <div class="col-md-12">
                <div class="card shadow-sm">
                    <div class="card-header p-4 bg-white text-white">
                        <h5 class="card-title mb-0 fw-bold text-black"><i class="bi bi-database me-2"></i> Database Backup
                        </h5>
                    </div>
                                    ...
                                    @foreach ($projects as $project)
                                        <div class="form-check">
                                            <input class="form-check-input project-cb" type="checkbox" name="projects[]"
                                                id="project{{ $project->id }}" value="{{ $project->id }}" checked>
                                            <label class="form-check-label"
                                                for="project{{ $project->id }}">{{ $project->project_name }}</label>
                                        </div>
                                    @endforeach
                                    ...
                                    @foreach ([['all', 'All Users', 'fw-bold'], ['staff', 'Staff Only'], ['vendors', 'Vendors Only']] as $i => $user)
                                        <div class="form-check">
                                            <input class="form-check-input" type="radio" name="user_type"
                                                id="{{ $user[0] }}" value="{{ $user[0] }}"
                                                {{ $i == 0 ? 'checked' : '' }}>
                                            <label class="form-check-label {{ $user[2] ?? '' }}"
                                                for="{{ $user[0] }}">{{ $user[1] }}</label>
                                        </div>
                                    @endforeach
                            ...
                            <div class="col-md-9">
                                ...
                                <div class="row" id="backupsList">
                                    @forelse($backups as $backup)
                                        <div class="col-md-4 mb-4 backup-item" data-date="{{ $backup['date'] }}"
                                            data-name="{{ $backup['name'] }}" data-size="{{ $backup['sizeBytes'] }}">
                                            <div class="card backup-card">
                                                <div class="card-body text-center">
                                                    @if ($backup['type'] == 'xlsx' || $backup['type'] == 'excel')
                                                        <i class="mdi mdi-file-excel text-success"
                                                            style="font-size: 3rem;"></i>
                                                    @elseif($backup['type'] == 'csv')
                                                        <i class="mdi mdi-file-delimited text-info"
                                                            style="font-size: 3rem;"></i>
                                                    @else
                                                        <i class="mdi mdi-database text-warning"
                                                            style="font-size: 3rem;"></i>
                                                    @endif

                                                    <h6 class="card-title text-truncate mt-2"
                                                        title="{{ $backup['name'] }}">
                                                        {{ $backup['name'] }}
                                                    </h6>
                                                    ...
                                                    <div class="d-flex justify-content-center gap-2 mt-3">
                                                        <a href="{{ route('backup.download', $backup['name']) }}"
                                                            class="btn btn-icon btn-info download-btn" data-toggle="tooltip"
                                                            title="Download">
                                                            <i class="mdi mdi-download"></i>
                                                        </a>
                                                        <button class="btn btn-icon btn-danger delete-btn"
                                                            data-file="{{ $backup['name'] }}" data-toggle="tooltip"
                                                            title="Delete">
                                    ...
                                    @empty
                                        <div class="col-12">
                                            <p class="text-center text-muted py-5">No backups found</p>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#backupForm').on('submit', function(e) {
                ...
                btn.html('<i class="mdi mdi-loading mdi-spin me-1"></i> Creating...').prop('disabled',
                true);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showAlert(response.message || 'Failed to create backup', 'danger');
            ...
            // Download functionality - handled by link href, no need for click handler

            // Delete functionality
            $(document).on('click', '.delete-btn', function() {
                const fileName = $(this).data('file');
                const $item = $(this).closest('.backup-item');
                            ...
                            if (response.success) {
                                $item.fadeOut(300, function() {
                                    ...
                                    if ($('.backup-item').length === 0) {
                                        $('#backupsList').html(
                                            '<div class="col-12"><p class="text-center text-muted py-5">No backups found</p></div>'
                                            );
                                    }
                                });
                                showAlert(`${fileName} deleted successfully`, 'success');
                            } else {
                                showAlert(response.message || 'Failed to delete backup',
                                    'danger');
                            }
                        },
                        error: function(xhr) {
                            const message = xhr.responseJSON?.message ||
                                'An error occurred while deleting backup';
                            showAlert(message, 'danger');
                        }
                    });
                }
