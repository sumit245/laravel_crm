...
@section('content')
  <div class="container p-2">
        ...
        ]" :addRoute="route('uservendors.create')" addButtonText="Add New Vendor"
            :exportEnabled="true" :importEnabled="true" :importRoute="route('import.vendors')" :importFormatUrl="route('vendors.importFormat')" 
            :bulkDeleteEnabled="true" :bulkDeleteRoute="route('uservendors.bulkDelete')"
            :deleteRoute="route('uservendors.destroy', ':id')" 
            :editRoute="route('uservendors.edit', ':id')" 
            :viewRoute="route('uservendors.show', ':id')" 
            pageLength="50" 
        ...
        @foreach ($vendors as $member)
                @php
                    $projectNames = $member->projects->pluck('project_name')->filter()->join(', ') ?: ($member->project_id ? ($member->project ? $member->project->project_name : '') : '');
                ...
                @endphp
                <tr data-project="{{ $projectNames }}" data-district="{{ $districtNames }}" data-manager="{{ $managerName }}">
                    <td>
            ...
            <td>{{ $member->lastName }}</td>
                ...
                <i class="mdi mdi-pencil"></i>
                        ...
                        <button type="button" class="btn btn-icon btn-danger delete-row" data-toggle="tooltip"
                            title="Delete Vendor" data-id="{{ $member->id }}"
    ...
    <div class="modal fade" id="bulkAssignProjectsModal" tabindex="-1" aria-labelledby="bulkAssignProjectsModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        <label for="project_ids" class="form-label">Select Projects <span class="text-danger">*</span></label>
                    ...
                    <div class="form-group">
                        ...
                        <div class="d-flex flex-column gap-2">
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="assignment_mode" id="mode_add" value="add" checked>
                                <label class="form-check-label" for="mode_add">
                                    <strong>Add Projects</strong> - Add selected projects to existing assignments (vendor will work on all projects)
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="assignment_mode" id="mode_replace" value="replace">
                                <label class="form-check-label" for="mode_replace">
                                    <strong>Replace Projects</strong> - Remove all existing project assignments and assign only selected projects
                                </label>
                            </div>
                        </div>
                    </div>
  ...
  <script>
    $(document).ready(function() {
            ...
            setTimeout(function() {
                ...
                // Store original applyFilters if it exists
                const originalApplyFilters = window.applyFilters;
                
                // Custom applyFilters function
                function applyCustomFilters() {
                    // Clear previous filters
                    customFilterFunctions = [];
                    vendorsTable.search('').columns().search('');
                    
                    // Get filter values
                    const projectFilter = $('.filter-select[data-filter="project"]').val();
                    const districtFilter = $('.filter-select[data-filter="district"]').val();
                    const managerFilter = $('.filter-select[data-filter="manager"]').val();
                    
                    // Create custom filter functions based on data attributes
                    if (projectFilter) {
                        customFilterFunctions.push(function(settings, data, dataIndex) {
                            if (settings.nTable.id !== 'vendorsTable') return true;
                            const $row = $(vendorsTable.row(dataIndex).node());
                            const rowProject = $row.data('project') || '';
                            return rowProject.includes(projectFilter);
                        });
                    }
                    ...
                    if (districtFilter) {
                        customFilterFunctions.push(function(settings, data, dataIndex) {
                            ...
                            return rowDistrict.includes(districtFilter);
                        });
                    }
                    ...
                    if (managerFilter) {
                        customFilterFunctions.push(function(settings, data, dataIndex) {
                            if (settings.nTable.id !== 'vendorsTable') return true;
                            const $row = $(vendorsTable.row(dataIndex).node());
                            const rowManager = $row.data('manager') || '';
                            return rowManager === managerFilter;
                        });
                    }
                ...
                // Override the applyFilters button click handler
                ...
                $('#clearFilters').off('click').on('click', function(e) {
                    ...
                    vendorsTable.search('').columns().search('').draw();
                });
                ...
                if ($('#bulkActions').length && $('#bulkDeleteBtn').length) {
                    ...
                    const $buttonContainer = $('<div class="d-flex flex-column gap-2 w-100 w-sm-auto"></div>');
                    ...
                    let assignProjectsBtn = $('<button type="button" class="btn btn-sm btn-primary d-inline-flex align-items-center gap-1 w-100" id="assignProjectsBtn" style="display: none;"><i class="mdi mdi-link-variant"></i><span>Assign Projects</span></button>');
            ...
            $.ajax({
              ...
              data: {
                    ...
                    success: function(response) {
                        const modal = bootstrap.Modal.getInstance(document.getElementById('bulkAssignProjectsModal'));
                        if (modal) {
                            modal.hide();
                        }
                        Swal.fire({
              ...
              error: function(xhr) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error!',
                            text: xhr.responseJSON?.message || 'Failed to assign projects. Please try again.',
                            confirmButtonText: 'OK'
                        });
                    },
                    complete: function() {
                        $('#confirmAssignProjects').prop('disabled', false).html('<i class="mdi mdi-check"></i> Assign Projects');
          }
        });
      });
    });
