...
<!-- Commneted columns need to be added in the table --> @if ($inventory->isEmpty()) <p>No inventory available for this store.</p> @else <table id="viewInventoryTable" :pageLength="50" class="table-striped table-bordered table-sm table m-2">
  <thead>
    <tr>
      <th>#</th> @if ($projectType == 1) <th>Item Code</th>
      <th>Item Name</th>
      <!-- <th>Manufacturer</th> -->
      <th>Make</th>
      <th>Model</th>
      ...
      <th>Quantity</th>
      <th>Unit Price</th>
      <th>Total Value</th> @endif <th>Actions</th>
    </tr>
  </thead>
  <tbody> @foreach ($inventory as $item) <tr>
      <td>{{ $loop->iteration }}</td> @if ($projectType == 1) <td>{{ $item->item_code }}</td>
      <td>{{ $item->item }}</td>
...
<script>
  function printTable() {
    ...
    newWindow.document.write(' < html > < head > < title > Print Inventory < /title> < /head> < body > ');
        newWindow.document.write(printContents); newWindow.document.write('</body> < /html>');
          newWindow.document.close(); newWindow.print();
        }

        function exportToCSV() {
          const rows = [
            @if($projectType == 1)['Item Code', 'Item Name', 'Manufacturer', 'Make', 'Model', 'Serial Number', 'HSN Code', 'Unit', 'Rate', 'Quantity', 'Total Value', 'Description', 'Received Date']
            @else['Category', 'Sub Category', 'Item Name', 'Quantity', 'Unit Price', 'Total Value']
            @endif
          ];
        ...
        window.onload = function() {
          const table = $('#viewInventoryTable').DataTable({
            dom: "<'row'<'col-sm-6 d-flex align-items-center'f><'col-sm-6 d-flex justify-content-end'B>>" + "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-5'i><'col-sm-7'p>>",
            buttons: [{
              extend: 'collection',
          ...
          $('#viewInventoryTable tbody').on('click', 'input[type="checkbox"]', function() {
            const allChecked = $('#viewInventoryTable tbody input[type="checkbox"]:checked').length === table.rows().count();
